  ...
  return (
    <div className="px-4 pt-4">
      <div className="flex items-center gap-3 mb-4">
        <h1 className="text-2xl font-bold">TM6 Recetas</h1>
        <p className="text-sm text-muted-foreground">{filtered.length.toLocaleString()}</p>
      </div>
              ...
              <div>
                ...
                <div className="flex gap-1.5">
                  {(['fácil', 'media', 'avanzada'] as const).map(d => (
                    <Badge
                      key={d}
                      variant={filter.difficulty === d ? 'default' : 'outline'}
                      className="cursor-pointer"
                      onClick={() => setFilter({ difficulty: filter.difficulty === d ? null : d })}
                    >
                      {DIFFICULTY_LABELS[d]}
                    </Badge>
                  ))}
              ...
              <div>
                ...
                <div className="flex gap-1.5">
                  {[15, 30, 60, 120].map(t => (
                    <Badge
                      key={t}
                      variant={filter.maxTime === t ? 'default' : 'outline'}
                      className="cursor-pointer"
                      onClick={() => setFilter({ maxTime: filter.maxTime === t ? null : t })}
                    >
                      {t < 60 ? `${t}′` : `${t / 60}h`}
                    </Badge>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(Object.entries(ACCESSORY_LABELS) as [string, string][]).map(([k, v]) => (
                    <Badge
                      key={k}
                      variant={filter.utensils.includes(k as any) ? 'default' : 'outline'}
                      className="cursor-pointer"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['vegano', 'vegetariano', 'sin_gluten', 'sin_lactosa', 'fit', 'rapida', 'economica'].map(t => (
                    <Badge
                      key={t}
                      variant={filter.tags.includes(t as any) ? 'default' : 'outline'}
                      className="cursor-pointer"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      <Clock size={12} />{timeStr(recipe.totalTime)}
                    </span>
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      <ChefHat size={12} />{DIFFICULTY_LABELS[recipe.difficulty]}
                    </span>
